...
class BillDetailPresenter {
  static BillDetailViewModel presentBillDetail(
      {required BillsState billState, required String billId}) {
    if (billState is! BillsFetchedState) {
      throw Exception(
          'BillDetailPresenter: billState is not BillsFetchedState');
    }
    ...
    return BillDetailViewModel(
        bill: bill, transactionsLoaded: transactionsLoaded);
  }
}
...
class BillDetailViewModel extends Equatable {
  ...
  const BillDetailViewModel(
      {required this.bill, required this.transactionsLoaded});
