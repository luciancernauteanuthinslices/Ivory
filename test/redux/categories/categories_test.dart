...
void main() {
  ...
  test(
      "When asking to fetch for categories it should firstly display a loading state",
      () async {
    //given
    ...
    final appState = store.onChange.firstWhere(
        (element) => element.categoriesState is CategoriesLoadingState);
    //when
  ...
  test("When fetching categories fails it should display an error state",
      () async {
    //given
    final store = createTestStore(
    ...
    final appState = store.onChange.firstWhere(
        (element) => element.categoriesState is CategoriesErrorState);
    //when
    store.dispatch(GetCategoriesCommandAction());
    //then
  ...
  test(
      "When fetching categories successfully it should display a categories list",
      () async {
    //given
    final store = createTestStore(
        categoriesService: FakeCategoryService(),
    ...
    final appState = store.onChange.firstWhere(
        (element) => element.categoriesState is CategoriesFetchedState);
    //when
    store.dispatch(GetCategoriesCommandAction());
    //then
    final CategoriesFetchedState categoriesFetchedState =
        (await appState).categoriesState as CategoriesFetchedState;
