...
void main() {
  test("When asking to fetch cities, state should change to loading", () async {
    ...
    final appState = store.onChange.firstWhere(
      (element) => element.citySuggestionsState is CitySuggestionsLoadingState,
    );
    ...
    // then
    expect(
      (await appState).citySuggestionsState,
  ...
  test("When cities are fetched, state should change to fetched", () async {
    ...
    final appState = store.onChange.firstWhere(
      (element) => element.citySuggestionsState is CitySuggestionsFetchedState,
    );
    ...
    // then
    expect(
      (await loadingState).citySuggestionsState,
      isA<CitySuggestionsLoadingState>(),
    );
    expect(
      (await appState).citySuggestionsState,
  ...
  test(
    "When failed fetching cities, the state should change to error",
    () async {
      // given
      final store = createTestStore(
