...
void main() {
  ...
  const addressSuggestion =
      AddressSuggestion(address: 'address', city: 'city', country: 'DE');

  group("Date & place of birth", () {
    test(
        "When user submits the date and place of birth info, the state should be updated",
        () async {
      // given
      ...
      final appState = store.onChange.firstWhere((state) =>
          state.onboardingPersonalDetailsState.attributes.hasBirthInfo);
      ...
      // then
      final onboardingPersonalDetailsState =
          (await appState).onboardingPersonalDetailsState;
      ...
      expect(
          onboardingPersonalDetailsState.attributes.nationality, nationality);
    });
  });

  group("Address of residence", () {
    test("When user has selected the address, the state should be updated",
        () async {
      // given
      final store = createTestStore(
        initialState: createAppState(
      ...
      expect(onboardingPersonalDetailsState.attributes.selectedAddress,
          addressSuggestion);
    });

    test(
        "When the user is searching for other address, selected address should be removed",
        () async {
      // given
      ...
      final store = createTestStore(
        onboardingPersonalDetailsService:
            FakeOnboardingPersonalDetailsService(),
        initialState: createAppState(
          authState: authInitializedState,
          onboardingPersonalDetailsState:
              const OnboardingPersonalDetailsState(attributes: attributes),
        ),
      );

      final appState = store.onChange.firstWhere((state) =>
          state.onboardingPersonalDetailsState.attributes.selectedAddress ==
          null);
  ...
  group("Person creation", () {
    test(
        "When the person is successfully created, <isAddressSaved> should change to true",
        () async {
      // given
      const attributes = OnboardingPersonalDetailsAttributes(
        birthDate: birthDate,
      ...
      final loadingState = store.onChange.firstWhere(
          (state) => state.onboardingPersonalDetailsState.isLoading);
      final appState = store.onChange.firstWhere((state) =>
          state.onboardingPersonalDetailsState.isAddressSaved == true);

      // when
      store.dispatch(
          CreatePersonAccountCommandAction(addressLine: "", houseNumber: ""));

      // then
      expect(
          (await loadingState).onboardingPersonalDetailsState.isLoading, true);
    ...
    test(
        "When the person failed to create, <isAddressSaved> should be false and <errorType> should not be null",
        () async {
      // given
      const attributes = OnboardingPersonalDetailsAttributes(
        birthDate: birthDate,
      ...
      final store = createTestStore(
        onboardingPersonalDetailsService:
            FakeFailingOnboardingPersonalDetailsService(),
        initialState: createAppState(
          authState: authInitializedState,
          onboardingPersonalDetailsState:
              const OnboardingPersonalDetailsState(attributes: attributes),
      ...
      final loadingState = store.onChange.firstWhere(
          (state) => state.onboardingPersonalDetailsState.isLoading);
      final appState = store.onChange.firstWhere((state) =>
          state.onboardingPersonalDetailsState.isAddressSaved == false);
      ...
      expect(onboardingPersonalDetailsState.attributes, attributes);
      expect(onboardingPersonalDetailsState.errorType,
          OnboardingPersonalDetailsErrorType.unknown);
    });
  });

  group("Create/confirm mobile number", () {
    test("When mobile number is created, tanRequestedAt should change",
        () async {
      //given
      const attributes = OnboardingPersonalDetailsAttributes(
        birthDate: birthDate,
      ...
      final appState = store.onChange.firstWhere((state) =>
          ...
          state.onboardingPersonalDetailsState.attributes.mobileNumber ==
              '123456');
    ...
    test(
        "When mobile number confirmation happens, isMobileConfirmed should be true",
        () async {
      //given
      const attributes = OnboardingPersonalDetailsAttributes(
        birthDate: birthDate,
      ...
      final loadingState = store.onChange.firstWhere(
          (state) => state.onboardingPersonalDetailsState.isLoading);
      final appState = store.onChange.firstWhere((state) =>
          state.onboardingPersonalDetailsState.isMobileConfirmed == true);

      // when
      store.dispatch(ConfirmMobileNumberCommandAction(
          mobileNumber: '+15550101', token: '212212'));
