...
void main() {
  ...
  final authInitializedState = AuthenticationInitializedState(
    mockUser,
    AuthType.onboarding,
  );
  ...
  group("Date & place of birth", () {
    test(
      ...
      () async {
        ...
        expect(
          onboardingPersonalDetailsState.attributes.nationality,
          nationality,
        );
      },
    );
  ...
  group("Address of residence", () {
    test(
      "When user has selected the address, the state should be updated",
      () async {
        // given
        final store = createTestStore(
          initialState: createAppState(
        ...
        expect(
          onboardingPersonalDetailsState.attributes.selectedAddress,
          addressSuggestion,
        );
      },
    );
  ...
  group("Person creation", () {
    test(
      ...
      () async {
        ...
        // then
        expect(
          (await loadingState).onboardingPersonalDetailsState.isLoading,
          true,
        );
    ...
    test(
      ...
      () async {
        ...
        expect(onboardingPersonalDetailsState.attributes, attributes);
        expect(
          onboardingPersonalDetailsState.errorType,
  ...
  group("Create/confirm mobile number", () {
    test(
      "When mobile number is created, tanRequestedAt should change",
      () async {
        //given
        const attributes = OnboardingPersonalDetailsAttributes(
          birthDate: birthDate,
        ...
        expect(
          onboardingPersonalDetailsState.attributes.mobileNumber,
          '123456',
        );
      },
    );
