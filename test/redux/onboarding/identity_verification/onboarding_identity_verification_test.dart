...
void main() {
  ...
  test(
    'when creating urlForIntegration it should display loading state',
    () async {
      //given
      ...
      //when
      store.dispatch(
        CreateIdentificationCommandAction(accountName: accountName, iban: iban),
      );
      //then
  ...
  test(
    'when created urlForIntegration successful should update with success',
    () async {
      //given
      final store = createTestStore(
        onboardingIdentityVerificationService:
  ...
  test(
    'when created urlForIntegration unsuccessful should update with error',
    () async {
      //given
      final store = createTestStore(
        onboardingIdentityVerificationService:
      ...
      expect(identityVerificationState.isLoading, false);
      expect(
        identityVerificationState.errorType,
  ...
  group("Bank identification", () {
    ...
    test(
      ...
      () async {
        ...
        final documentsFetchedState = store.onChange.firstWhere(
          (state) =>
              state.documentsState is DocumentsFetchedState &&
              (state.documentsState as DocumentsFetchedState)
                  .documents
                  .isNotEmpty,
        );
        ...
        final documentsState =
            (await documentsFetchedState).documentsState
                as DocumentsFetchedState;

        expect(loadingState.isLoading, true);
        expect(identityVerificationState.isLoading, false);
        expect(
          identityVerificationState.status,
  ...
  group('fetching credit limit', () {
    ...
    test(
      'when credit limit was ordered it should display loading state',
      () async {
        //given
        final store = createTestStore(
          onboardingIdentityVerificationService:
    ...
    test(
      'when credit limit is successful fetched it should be displayed',
      () async {
        //given
        final store = createTestStore(
          onboardingIdentityVerificationService:
    ...
    test(
      "when fetch credit limit has failed it should update with error",
      () async {
        //given
        final store = createTestStore(
          onboardingIdentityVerificationService:
