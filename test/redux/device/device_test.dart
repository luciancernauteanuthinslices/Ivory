...
void main() {
  ...
  group('Creating device binding', () {
    test(
      'When creating device binding successfully should be succesful',
      () async {
        // given
        ...
        // when
        store.dispatch(CreateDeviceBindingCommandAction());

        // then
        expect(
          (await loadingState).deviceBindingState,
          isA<DeviceBindingLoadingState>(),
        );
        expect(
          (await appState).deviceBindingState,
    ...
    test(
      'When creating device binding is failing should update with error',
      () async {
        // given
        final store = createTestStore(
  ...
  group('Deleting device binding', () {
    test(
      'When deleting device binding successfully should be succesful',
      () async {
        // given
        final store = createTestStore(
          deviceBindingService: FakeDeviceBindingService(),
        ...
        // when
        store.dispatch(DeleteBoundDeviceCommandAction(deviceId: 'deviceId'));
    ...
    test(
      'When deleting device binding is failing should update with error',
      () async {
        // given
        final store = createTestStore(
          deviceBindingService: FakeFailingDeviceBindingService(),
  ...
  group("Verifying device binding", () {
    test(
      'When verifying device binding successfully should be succesful',
      () async {
        // given
        final store = createTestStore(
          deviceBindingService: FakeDeviceBindingService(),
    ...
    test(
      'When verifying device binding is failing should update with error',
      () async {
        // given
        final store = createTestStore(
          deviceBindingService: FakeFailingDeviceBindingService(),
  ...
  test(
    'When fetching bound devices fails, should return error with empty list',
    () async {
      //given
      final store = createTestStore(
        deviceBindingService: FakeFailingDeviceBindingService(),
  ...
  group('Check if binding is possible', () {
    test(
      'When checking if binding is possible succesfully, should return true',
      () async {
        //given
        final store = createTestStore(
          deviceBindingService: FakeDeviceBindingService(),
          ...
          initialState: createAppState(
            deviceBindingState: DeviceBindingFetchedState(
              devices,
              devices[0],
              false,
              false,
        ...
        //when
        store.dispatch(DeviceBindingCheckIfPossibleCommandAction());
