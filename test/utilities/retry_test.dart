...
void main() {
  test(
    'retry function should return the result if no exception is thrown',
    () async {
      // given
      int attempt = 0;
      int callback() {
      ...
      // when
      final result = await retry(callback);
      // then
      expect(result, 1);
    },
  );
  ...
  test(
    "retry function should retry if retryIf is a Future and returns true",
    () async {
      // given
      int attempt = 0;
      // when
